...
$(document).ready(function () {
    ...
    function CheckInput(config) {
        ...
        else if (placement.is(".btn-group")) MassiveAjax(field, config).done(function (data) { DataToButtons(data, config); });
    ...
    function DataToDropDown(jdata, config) {
        ...
        placement.append($("<option/>").text("Please choose a value"))
        if (config.data("text") == undefined || config.data("value") == undefined) alert("Dropdown Binding requires data-text, data-value fields to be set.");
            ...
            $.each(data, function (i, o) {
                ...
                if (text.indexOf(',') > 0) placement.append($('<option/>').attr("value", o).text(o.replace(" ", "") + " : " + o.replace(" ", "")));
                else placement.append($('<option/>').attr("value", o).text(o));
            });
        }
    };
    ...
    function DataToDiv(data, config) {
        var placement = $("#" + config.data("results"));
        var data = data.d;
    ...
    function DataToTable(data, config) {
        ...
        if (data.d.length > 0) {
            ...
            placement.createTable($.parseJSON(data.d));
    ...
    function DataToButtons(data, config) {
        $("#loadingModal").hide();
        __config = config;
        var placement = $("#" + config.data("results"));
        var data = data.d;
        if (data.length > 0) {
            $.each(data, function (idx, item) {
                placement.append('<a class="btn btn-xs btn-primary" href="#" onclick="Selected(\''+item+'\')"><i class="fa fa-database"></i>&nbsp;' + item + '</a>');
            });
        };
    };
    function CreateHiddenControls(data, config) {
        ...
        var data = data.d;
        $.each(data, function (idx, item) {
            placement.append("<input type='hidden' id='hdn" + idx + "' value='" + item + "'/>");
        });
    };
    ...
    function MassiveAjax(fieldstr, config, callback) {
        ...
        var access_id = config.data("accessid");

        var msg = "Missing data-attributes for: ";
        var massive_required = [];

        if (method !== "MassiveInsert" && method != "MassiveForm" && method !== "MassiveQuery") {
        ...
        $("#loadingModal").show();
        if (method !== "MassiveForm" && method !== "MassiveQuery") {
        ...
        if (method == "MassiveForm") var json_data = JSON.stringify({ "rform": "[{" + JSON.stringify($("form").serialize()) + "}]", "table": table });
        if (method == "MassiveQuery") var json_data = JSON.stringify({ "query": query, "table": table, "access_id": access_id });
        if (method == "MassiveColumns") var json_data = JSON.stringify({ "table": table });
        if (method == "MassiveTables") var json_data = JSON.stringify({});
        return $.ajax({
            url: "/assets/services/Massive.svc/" + method,
